...
void exec(SOCKET client) {
    ...
    while (true) {
        ...
        if (recv(client, decrypted, 7, 0) > 0) {
            ...
            unsigned int packetLen = Stream.readInt();
            unsigned short packetVer = Stream.readShort();

            cout << "[*] Got packet with Id: " << packetId << " || Length: " << packetLen << " || Version: " << packetVer << endl;
...
int main() {
    ...
    cout << "[*] Server listening at 0.0.0.0:" << port << endl;
