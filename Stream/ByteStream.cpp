...
void ByteStream::writePacket(const uint16_t id, int32_t sock, uint16_t ver) {
    // RC4__init();
    // PepperCrypto__init();

    // RC4__encrypt(buffer.data(), buffer.data());
	// PepperCrypto__encrypt(id, buffer.data(), buffer.size());
